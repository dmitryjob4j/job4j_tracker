...
public class Matches {
    public static void main(String[] args) {
        ...
        int gamerNum = 11;
        int select = 0;
        System.out.println("+++Доброй пожаловать в игру 11 спичек.+++" + "\n" +
                "+++Условия игры: Играют двое Господ.+++" + "\n" +
                "+++Каждый Господин может брать от 1-й до 3-х спичек+++" + "\n" + "+++Начинаем+++");
        while (match > 0) {
            ...
            if (select <= 3 && select > 0) {
                match -= select;
                if (match <= 0) {
                    System.out.print("Господин №" + gamer + " выиграл игру!" + "\n" + "Игра окончена");
                    break;
                } else {
                    gamerNum--;
